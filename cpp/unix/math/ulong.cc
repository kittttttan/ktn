 ...
 */
void ULong::alloc(size_t length, bool zero)
{
    if (l_ != length) {
        c_ = l_ = length;
        delete [] d_;
...
ULong ULong::operator<<(uddigit n) const
{
    const size_t l = l_;
...
/*!
 ...
 @return this ^ n
 */
ULong ULong::pow(uddigit n) const
{
    ULong p(1);
    ULong a(*this);
 ...
 */
ULong ULong::fact_odd(uddigit n)
{
    ULong m = ONE;
    uddigit mi, mj;
    uddigit i, j;
 ...
 */
ULong ULong::fact_even(uddigit n)
{
    uddigit s = 0;
...
/*!
 ...
 @return n!
 */
ULong ULong::factorial(uddigit n)
{
    //ULong u = ONE;
    //for (int i = 2; i <= n; ++i) {
    //    u *= i;
